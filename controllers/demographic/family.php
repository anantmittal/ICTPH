...
class Family extends CI_Controller
{
  ...
  function add_photos($policy_id) {
    ...
    $this->session->set_userdata('msg',$message);
    redirect('/admin/enrolment/index');
    //    $this->load->view('demographic/upload_success', $this->data);
  }
...
//	  $var_rh = 'member'.$i.'_rh_factor';
//	  if(($person_rec->blood_group != $posted_data[$var_bg]) || ($person_rec->rh_factor != $posted_data[$var_rh]))
	  if($person_rec->blood_group != $posted_data[$var_bg])
	  {
	    $person_rec->blood_group = $posted_data[$var_bg];
//	    $person_rec->rh_factor = $posted_data[$var_rh];
	    if(!($person_rec->save()))
	    {
  ...
  private function save_data($form_fields, $table_fields, $num_rows, $table_name) {
	...
	function add_soundex_to_persons()
	{
		$this->load->helper('soundex');
		$this->load->model('demographic/person','person');
		
		soundex_all($this->person,"full_name","name_soundex",'ret_normalized_name_parts');
	}
}
