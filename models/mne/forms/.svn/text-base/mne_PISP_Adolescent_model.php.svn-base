<?php 
class mne_pisp_adolescent_model extends IgnitedRecord 
{
	var $table = "mne_pisp_adolescent";
	var $subtables = array("mne_pisp_acute_condition", "mne_pisp_personal_illness");
	function get_id_for_org_id($org_id)
	{
		$ret = false;
		if($record = $this->find_by('adolescent_id',$org_id))
		{
			$ret = $record->id;
		}
		return $ret;
	...
	{
		$ret = array();
		if($records = $this->find_all_by('adolescent_id',$org_id))
		{
			foreach($records as $row)
				$ret[$row->id] = array($row->date_interview, 'adolescent', '12000');
		}
		return $ret;
	}
	...
	{
		$idfield = "adolescent_id";
		$datefield = "date_interview";
		if(!$this->find_by($idfield,$person_org_id))
			return null;
		if($record = $this->find_by_sql('select max(id) as id from '.$this->table.' where '.$idfield.'="'.$person_org_id.'" and '.$datefield.'=(select max(date_interview) as date from '.$this->table.' where '.$idfield.'="'.$person_org_id.'")' ))
		{
