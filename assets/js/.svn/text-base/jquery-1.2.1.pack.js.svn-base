			...
			if (this.value.length >= opts.minchar && $.trim(this.value)!="Search Terms") {
				...
				// new query detected
					...
					if ($(".jSuggestLoading").length==0)
						$('<div class="jSuggestLoading"><img src="'+image_url+opts.loadingImg+'" align="bottom" /> '+ opts.loadingText+'</div>').prependTo("#jSuggestContainer");
