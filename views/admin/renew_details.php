...
<form action="<?php echo $this->config->item('base_url').'index.php/admin/enrolment/renew_policy/'.$policy_number; ?>" method="POST">

<?php echo validation_errors(); ?>

 <table  width="100%" border="0" cellspacing="2" cellpadding="0" class="data_table">
<tr>
   <?php if($policy_number != NULL){ echo '<td> Previous Policy Number </td> <td> '.$policy_number.' </td>  <input type="hidden" name="prev_policy_number" value="'.$policy_number.'"> ';} ?> 
...
<!--     <td><input type="text" name="address" value="<?php echo $street_address;?>"/></td> -->
     <td>Village <?php echo form_dropdown("add_village", $villages, $village_id, 'class="bigselect"'); ?></td>
     <td>Taluka <?php echo form_dropdown("add_taluka", $talukas, $taluka_id, 'class="bigselect"'); ?></td>
     <td> District <?php echo form_dropdown("add_district", $districts, $district_id, 'class="bigselect"'); ?></td>
   </tr>
...
<?php 
  ...
  <tr class="row">
    ...
    <td width="2%"> <?php if ($i!=0) { ?><input type=checkbox name="member<?php echo $i;?>_del" value="Yes"/> </td> <?php }?>
    <td width="26%"> 
	<?php if($i ==0)
	      { echo $member->full_name; ?>
       <input type="hidden" name="member<?php echo $i;?>_name" value="<?php echo $member->full_name; ?>" /> 
	<?php	}
	else { ?>
       <input type="text" name="member<?php echo $i;?>_name" size=30 value="<?php echo $member->full_name; ?>"></input> <?php } ?>
       <input type="hidden" name="member<?php echo $i;?>_id" value="<?php echo $member->id; ?>" /> 
    </td>

    <td width="40%">
     <?php echo form_dropdown('member'.$i.'_ped', $peds, $member->addictions, 'class="bigselect"'); ?>
	...
	<?php if($i ==0)
	      { echo $member->gender; ?>
      <input type="hidden" name="member<?php echo $i;?>_gender" value="<?php echo $member->gender; ?>"></input>
	<?php	}
	else { ?>
      <input type="radio" name="member<?php echo $i;?>_gender" value="M" <?php if($member->gender == 'M'){echo "Checked";} ?> >M</input>
      <input type="radio" name="member<?php echo $i;?>_gender" value="F" <?php if($member->gender == 'F'){echo "Checked";} ?> >F</input>
	<?php } ?>
    </td>

    <td width="8%">
      <input type="text" name="member<?php echo $i;?>_dob" value="<?php echo Date_util::date_display_format($member->date_of_birth);?>" size=10 maxlength=10 />
    </td>

    <td width="9%">
	<?php if($i ==0)
	      { echo $member->relation; ?>
      <input type="hidden" name="member<?php echo $i;?>_relation" value="<?php echo $member->relation; ?>"></input>
	<?php	}
	else { ?>
     <?php echo form_dropdown('member'.$i.'_relation', $relations, $member->relation, 'class="bigselect"'); ?> <?php } ?>
    </td>

    <td width="7%">
     <?php echo form_dropdown('member'.$i.'_bg', $bgs, $member->blood_group, 'class="bigselect"'); ?>
    </td>
  </tr>
<?php $i++; 
  }
//  for ($i=0, $i < 7, $i++) {
  ...
  <tr class="row">
    ...
    <td width="2%">  </td>
    <td width="26%"> 
       <input type="text" name="member<?php echo $i;?>_name" size= 30 value="" /> 
    </td>
    ...
    <td width="8%">
      <input type="text" name="member<?php echo $i;?>_dob" value="DD/MM/YYYY" size=10 maxlength=10 />
    </td>
