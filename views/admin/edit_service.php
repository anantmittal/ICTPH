...
//Creating javascript array from php array.
	...
	if(isset($edit_service)){?>
		...
		medication_list = [      
		<?php
		foreach ($medications_list as $medications){
			$id = $medications->id;
			$medications_name = $medications->name.'-'.$medications->generic_name;;
			$retail_unit=$medications->retail_unit;
			$purchase_unit=$medications->purchase_unit;
			$generic_name = $medications->generic_name;
			$quantity = $medications->capacity;
		?>
		{id: '<?php echo $id; ?>', name: '<?php echo $medications_name; ?>', generic_name: '<?php echo $generic_name; ?>', capacity: '<?php echo $quantity; ?>', retail_unit: '<?php echo $retail_unit; ?>', purchase_unit: '<?php echo $purchase_unit; ?>'},
		<?php
		}
		?>
		];
		...
		$(document).ready(function(){
			$("#selected_consumable").user_autocomplete(consumable_list,'consumable_product_id','','consumable_retail_unit','consumable_purchase_unit');
			$("#selected_medication").local_autocomplete(medication_list,'medication_product_id','','medication_retail_unit','medication_purchase_unit');
		});
	<?php }?>
</script>
</head>
	...
	<div id="main">
		...
		<div id="leftcol">
			...
			<div class="yelo_body" style="padding: 8px;">
				...
				<div class="action_items" style="font-weight: bold"><a href="<?php echo $this->config->item('base_url');?>index.php/admin/consumables_configuration/find_service/edit">Edit Service</a></div>
			...
			<div class="blue_left">
				<div class="blue_right">
					<div class="blue_middle">
						<span class="head_box">Edit Service</span>
					</div>
				</div>
			</div>
			<div class="blue_body" style="padding: 8px;">
				<form id = "find_service_form" method="post" onSubmit="return onFindServiceSubmit('<?php echo $this->config->item('base_url')?>');">
								<?php $this->load->view('admin/find_service.php'); ?>
								
				</form>
				<form method="POST" id="add_service_table"
								action="<?php echo $this->config->item('base_url').'index.php/admin/consumables_configuration/service/edit'?>"
								onSubmit="return ValidateConsumable()">
					<table width="600" border="0" align="center" cellpadding="5"
						cellspacing="1" class="data_table" >
							<?php 
								if(isset($edit_service)){
									$this->load->view('admin/add_edit_service.php');
							 }?>
