...
$(document).ready(function(){
	...
	$("#add_root_parent_href").click(function() {
...
function populateRightContent(nodeId,nodeName,nodeType,parentId,parentName){
	...
	//Add hidden fileds with value
	//1.Nodeid,2.nodename,3.parentId
	$("#hidden_fields").html();
...
function deleteNode(){
	...
	if (r==true)
	  {
		document.forms["add_delete_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/ros/delete'?>";
...
function addNode(){
	if(validateElement()){
		document.forms["add_delete_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/ros/add'?>";
		document.forms["add_delete_node"].submit();
	}
}
...
function addRootNode(){
	if(validateParent()){
		document.forms["add_root_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/ros/add_root_element'?>";
	...
	<div id="main">
		<div id="leftcol">
			...
			<div class="yelo_body" style="padding: 8px;">
				...
				<div class="action_items" style="font-weight: bold"><a href="<?php echo $this->config->item('base_url');?>index.php/admin/visit_document_configuration/ros">ROS Configuration</a></div>
			...
			<div class="blue_left">
				<div class="blue_right">
					<div class="blue_middle">
						<span class="head_box">ROS Configuration</span>
					</div>
				</div>
			</div>
						...
						<tr class="gridrow">
						    <td id="ros_left_content" width="65%" valign="top" style="border-right:1px solid #ccc;">
							    <div id="treecontrol" style="display: block;">
									...
									<a href="javascript:void(0);" id="add_root_parent_href">Add root element</a>
								</div>
								<?php 
							    	...
							    	foreach ($review_of_system_tree->getRootParents() as $node) {
							    			printNode($node, $review_of_system_tree);
							    	}
									echo '</ul>';
			
									function printNode ($node, $review_of_system_tree) {
										echo '<li>';
										$node_id = $node->getUid();
										...
										if($parent_id == -1){
											$parent_id = "";
											$parent_name = "Root element";
										}else{
											$parent_name = $review_of_system_tree->getValue($parent_id);
										}
							    		...
							    		if(!empty($children)){
							    			...
							    			foreach ($children as $childUid) {
							    				...
							    				printNode($childnode, $review_of_system_tree);
							    			}
							    			echo "</ul>";
							    		}
										echo "</li>";
								...
								<form method="POST" id="add_delete_node">
									...
									<table width="100%" id="add_element_table" style="display: none">
										...
										<tr>
											...
											<td><?php $options = array(
	                							...
	                							//$js = 'id="node_types" onChange="onTypeChange();"';
								 ...
								 <form method="POST" id="add_root_node">
									<table width="100%" id="add_root_element_table">
										<tr><th valign="top" colspan="3">Add root element</th></tr>
										<tr>
