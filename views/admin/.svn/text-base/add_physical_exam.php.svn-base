...
function deleteNode(){
	...
	if (r==true)
	  {
		document.forms["add_delete_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/physical_exam/delete'?>";
...
function addNode(){
	if(validateElement()){
		document.forms["add_delete_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/physical_exam/add'?>";
		document.forms["add_delete_node"].submit();
	}	
}
...
function addRootNode(){
	if(validateParent()){
		document.forms["add_root_node"].action = "<?php echo $this->config->item('base_url').'index.php/admin/visit_document_configuration/physical_exam/add_root_element'?>";
	...
	<div id="main">
		...
		<div id="leftcol">
			...
			<div class="yelo_body" style="padding: 8px;">
				...
				<div class="action_items" style="font-weight: bold"><a href="<?php echo $this->config->item('base_url');?>index.php/admin/visit_document_configuration/physical_exam">Physical Exam Configuration</a></div>
			...
			<div class="blue_left">
				<div class="blue_right">
					<div class="blue_middle">
						<span class="head_box">Physical Exam Configuration</span>
					</div>
				</div>
			</div>
						    ...
						    <td id="ros_left_content" width="65%" valign="top" style="border-right:1px solid #ccc;">
								...
								<?php 
							    	...
							    	foreach ($physical_exam_tree->getRootParents() as $node) {
							    			printNode($node, $physical_exam_tree);
							    	}
									echo '</ul>';
			
									function printNode ($node, $physical_exam_tree) {
										echo '<li>';
										$node_id = $node->getUid();
										...
										if($parent_id == -1){
											...
											$parent_name = $physical_exam_tree->getValue($parent_id);
										}
							    		...
							    		if(!empty($children)){
							    			...
							    			foreach ($children as $childUid) {
							    				...
							    				printNode($childnode, $physical_exam_tree);
							    			}
							    			echo "</ul>";
							    		}
										echo "</li>";
