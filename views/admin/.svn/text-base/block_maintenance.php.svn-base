	...
	<div id="main">
		...
		<div id="leftcol">
			...
			<div class="yelo_body" style="padding: 8px;">
				...
				<div class="action_items" style="font-weight: bold"><a href="<?php echo $this->config->item('base_url');?>index.php/admin/consumables_configuration/find_maintenance/block">Block Maintenance</a></div>
			</div>
			...
			<div class="blue_left">
				<div class="blue_right">
					<div class="blue_middle">
						<span class="head_box">Block/Unblock Maintenance</span>
					</div>
				</div>
			</div>
			<div class="blue_body" style="padding: 8px;">
				
					<form id = "find_maintenance_form_block" method="post" onSubmit="return onFindMaintenanceSubmitBlock('<?php echo $this->config->item('base_url')?>');">
								<?php $this->load->view('admin/find_maintenance.php'); ?>
					</form>
					<form method="POST"
									action="<?php echo $this->config->item('base_url').'index.php/admin/consumables_configuration/stock_maintenance/block_unblock'?>"
					...
					<table width="600" border="0" align="center" cellpadding="5"
						cellspacing="1" class="data_table" id="add_diagno_table">
									...
									<?php 
										if(isset($block_maintenance)){
										
										if($block_maintenance->status){
											$disabled="disabled";
										}		
										else{
											$disabled="";
									...
									?>
									<tr><td colspan="3" align="center"><strong>Block Maintenance</strong></td></tr>
									<tr>
										<td ><strong>Maintenance Name</strong></td>
										<td ><strong>:</strong></td>
										<td ><input type="text" name="maintenance_name" value="<?php echo $block_maintenance->name;?>" disabled/>
											<input type='hidden' name='maintenance_id' id='maintenance_id' value='<?php echo $block_maintenance->id;?>' />
										</td>
									</tr>
									
									<tr>
										<td colspan="3" align="right"><input type="submit" value="Block Maintenance"  id="block_maintenance" <?php  if(!empty($disabled)) echo ""; else echo "disabled"; ?>  > </input>
										<input type="submit" value="Unblock Maintenance"  id="unblock_maintenance " <?php  if(empty($disabled)) echo ""; else echo "disabled"; ?> > </input></td>
									</tr>
									<?php }?>
