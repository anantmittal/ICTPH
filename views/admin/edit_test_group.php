...
<script type="text/javascript" src="<?php echo "{$this->config->item('base_url')}assets/js/admin/add_test_group.js"; ?>"></script>
...
foreach ($test_list as $tests){
	$id = $tests->id;
	if(empty($tests->description)){
		$test_name = $tests->name;
	}else{
	   $test_name=$tests->name.'-'.$tests->description;
	
	}
	
	$test_cost = $tests->cost;
?>
{id: '<?php echo $id; ?>', name: '<?php echo $test_name;  ?>' , rate: '<?php echo $test_cost;  ?>'},
<?php
}
?>
];
	...
	<div id="main">
		...
		<div id="leftcol">
			...
			<div class="yelo_body" style="padding: 8px;">
				...
				<div class="action_items" style="font-weight: bold"><a href="<?php echo $this->config->item('base_url');?>index.php/admin/consumables_configuration/find_test_group/edit">Edit Test Group</a></div>
			...
			<div class="blue_left">
				<div class="blue_right">
					<div class="blue_middle">
						<span class="head_box">Edit Test Group</span>
					</div>
				</div>
			</div>
			<div class="blue_body" style="padding: 8px;">
				<form id = "find_test_group_form" method="post" onSubmit="return onTestGroupServiceSubmit('<?php echo $this->config->item('base_url')?>');">
								<?php $this->load->view('admin/find_test_group.php'); ?>
								
				</form>
				<form method="POST" id="add_test_group_table" action="<?php echo $this->config->item('base_url').'index.php/admin/consumables_configuration/test_groups/edit'?>" onSubmit="return ValidateTestGroup()">
					<table width="600" border="0" align="center" cellpadding="5" cellspacing="1" class="data_table" >
						<?php 
								if(isset($edit_test_group)){
									$this->load->view('admin/add_edit_test_group.php');
							 }?>
					</table>
					</form>
					<br />
