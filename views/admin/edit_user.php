...
<div id="main">
  ...
  <div id="leftcol">
		...
		<div class="yelo_body" style="padding:8px;">
		    ...
		    <div class="action_items" style="font-weight:bold"> <a href="<?php echo $this->config->item('base_url');?>index.php/admin/user_management/find_user/edit_user">Edit User </a></div>
		...
		<div class="blue_left">
			<div class="blue_right">
				<div class="blue_middle">
					<span class="head_box">Edit User</span>
				</div>
			</div>
		</div>
		<div class="blue_body" style="padding:8px;">
		<form id = "find_user_form" method="post" onSubmit="return onFindUserSubmit('<?php echo $this->config->item('base_url')?>');">
		<?php $this->load->view('admin/find_user.php'); ?>
		</form>
		<form method="POST" action="<?php echo $this->config->item('base_url').'index.php/admin/user_management/update_user'?>"
			onSubmit="return validateEditUserForm()" >		
		<?php 
			if(isset($edit_user)){
				$this->load->view('admin/add_edit_form.php');
			}
		?>
		</form>
		</div>
